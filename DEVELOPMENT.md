...
- **卡片版面与配色**（v1.3.6 重画，改之前先看下面这几条教训）：
  - 结构固定为「小标签 → **当前模型（标题区主角，31px，最多两行）** → 一行元信息 →
    候选列表 → 底部提示」；当前模型必须留在标题区，别再挪进信息条（v1.3.5 挪过，被吐槽「变蠢」）；
  - **别用大面积高饱和渐变 / 别堆彩色填充胶囊**：头部只是一层几乎看不见的主题淡色，
    标签用描边胶囊，只有「当前使用」是实心 —— 这两条是「廉价感」的主要来源；
  - 配色集中在 `model_card.THEMES`，每套只有 `{accent, soft, tint}` 三个值（其余全中性灰），
    配置 `model_card_theme` 只换强调色；改色只动那一处，别把颜色写回渲染函数里；
