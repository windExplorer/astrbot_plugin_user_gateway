  ...
  <n-config-provider :theme="theme" :theme-overrides="themeOverrides" :locale="zhCN" :date-locale="dateZhCN">
    <n-message-provider>
      <n-layout style="height: 100vh">
        <n-layout-header
          bordered
          style="padding: 12px 16px; display: flex; align-items: center; justify-content: space-between; gap: 8px"
        >
          <div style="display: flex; align-items: center; gap: 10px; min-width: 0">
            ...
            <n-tag v-else-if="syncTime && !isMobile" size="small" :bordered="false">同步于 {{ syncTime }}</n-tag>
          </div>
        ...
        <n-layout has-sider position="absolute" style="top: 57px">
          <n-layout-sider v-if="!isMobile" bordered :width="180" :native-scrollbar="false">
            <n-menu :value="activeKey" :options="menuOptions" :root-indent="18" />
          </n-layout-sider>

          <n-layout-content
            :content-style="isMobile ? 'padding: 12px 12px 24px' : 'padding: 18px 20px 28px'"
