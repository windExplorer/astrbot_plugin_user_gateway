...
等级可以指定 **主模型（提供商）**、可选的 **模型名**、以及 **备用模型（提供商）**：
...
- 备用被启用时**不沿用主提供商的模型名**（模型名可能不存在于备用提供商上），
  交回备用自己的默认模型 —— 与 AstrBot 自己切换提供商时的行为一致。
- 提供商列表只列**已加载**的（`/providers`）；未知 id 绝不会被下发 ——
  AstrBot 遇到不存在的提供商 id 会直接放弃本次请求。
